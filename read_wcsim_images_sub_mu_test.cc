...
int read_wcsim_images_sub_mu_test(const char* root_file, const char* image_file, const char* evt_file, const char* console_output, int start_event_no, int end_event_no, bool verbose = true, bool electron = false)
{
    ...
    if (!if_exists_event.good() && !if_exists_out.good()) {
        ...
        console_output << "Appending to existing image/event info files." << endl;
        ...
        console_output << "Error when opening output text file. Quit." << endl;
        return -1;
    }
    ...
    if (!file->IsOpen()){
        console_output << "Error, could not open input file: " << filename << endl;
        return -1;
    }
    ...
    if (geotree->GetEntries() == 1) console_output << "Confirmed... GeoTree has 1 entry." << endl;
    ...
    if (verbose) {
        console_output << "Total number of events: ";
        console_output << nevent;
        console_output << endl;
    }
    ...
    if (start_event_no > nevent || start_event_no < 1){
        console_output <<"Start event number not between 1 and nevent. Return" << endl;
        return -1;
    }
    
    if (end_event_no > nevent || end_event_no < 1){
        console_output <<"End event number not between 1 and nevent. Return" << endl;
        return -1;
    }
    ...
    for (int ev=start_event_no-1; ev<end_event_no; ev++){
        ...
        if (verbose){
            ...
            console_output << "Number of Sub-events (triggers) in current event: ";
            console_output << number_triggers;
            console_output << endl;
        }
        ...
        if(verbose){
            console_output << "Sub event number: ";
            console_output << index+1;
            console_output << endl;
            
            console_output << "Ncherenkovhits: ";
            console_output << ncherenkovhits;
            console_output << endl;

            console_output << "Ncherenkovdigihits: ";
            console_output << ncherenkovdigihits;
            console_output << endl;

            console_output << "Number of Tracks: ";
            console_output << wcsimrootevent->GetNtrack();
            console_output << endl;
        ...
        /*if (verbose){
            console_output << "Particle Vertex: (f, %f, %f) \n", particle_vertex(0), particle_vertex(1), particle_vertex(2));
            console_output <<"Particle Direction: (%f, %f, %f) \n", particle_direction(0), particle_direction(1), particle_direction(2));
            
            console_output <<"Energy: %f\n", track_energy);
            console_output <<"Ipnu: %d \n", ipnu);
            console_output <<"ID: %d \n", id);
        }*/
        ...
        if (!passed_cut) {
            console_output << "Failed cut, continue to next event. \n" << endl;
            continue;
        }
        ...
        for (int i=0;i<ncherenkovdigihits;i++)
        {
            ...
            if (hit_id.size() <= 0){
                console_output << "Hit ID array size non-positive, continue to next digitized charge: No. ";
                console_output << i;
                console_output << endl;
            ...
            if (i==0 && verbose) {
                console_output << "Number of Hits: ";
                console_output << hit_id.size();
                console_output << endl;
            ...
            for (int id=0; id<hit_id.size();id++){
                ...
                if (parent_id != 1){
                    flag = 0;
                    //console_output <<"ParentID/ParticleType are not 1/0. Not adding to image file. \n" << endl;
                    parentID_particletype++;
                    console_output << "Parent ID not 1. Raised flag no: ";
                    console_output << parentID_particletype+pmt_unfound;
                    console_output << endl;
                    break;
                }
                ...
                if (particle_type != 0){
                    ...
                    //console_output <<"ParentID/ParticleType are not 1/0. Not adding to image file. \n" << endl;
                    parentID_particletype++;
                    console_output << "Particle Type not 0. Raised flag no: ";
                    console_output << parentID_particletype+pmt_unfound;
                    console_output << endl;
                ...
                /*if (i==0 && verbose){
                 //console_output <<"x, y, z position: (%d, %d, %d) \n", pos[0], pos[1], pos[2]);
                 //console_output <<"x, y, z direction: (%d, %d, %d) \n", dir[0], dir[1], dir[2]);
                 
                 //console_output <<"Hit ID: %d \n", hit_id[id]);
                 
                 //ID of parent of particle (0 if primary)
                 //These are typically 1... assume 1 if primary
                 
                 //console_output <<"Parent ID: %d \n", parent_id);
                 
                 //Particle Type is 0 if primary
                 //console_output <<"Particle: %d \n", particle_type);
                 }*/
            ...
            if (abs(pmt_position[2]) < 1800){
                ...
                flag = 0;
                //console_output << "PMT Not found. Position: (%f, %f, %f) \n", pmt.GetPosition(0), pmt.GetPosition(1), pmt.GetPosition(2));
                
                console_output << "PMT Not found. Raised flag no: ";
                console_output << parentID_particletype+pmt_unfound;
                console_output << endl;
            ...
            /* if (i == 0 && verbose){
             console_output <<"q, t, tubeid: %f %f %d \n", curr_charge, t, tubeid, pmt_x, pmt_y, pmt_z);
             //console_output <<"Track parent ID: %d\n",track->GetParenttype());
             }*/
            if (i%100 == 0){
                
                console_output << "Cherenkov hit: ";
                console_output << i;
                console_output << endl;
            }
            
        } console_output << "End loop over digitized hits." << endl;// End of loop over Cherenkov digihits
        
        //} console_output <<"\n"); // Loop over triggers
        
        console_output << "Number of hits with ParentID != 1 or ParticleType != 0: ";
        console_output << parentID_particletype;
        console_output << endl;
        
        console_output << "Number of hits with PMTs not found: ";
        console_output << pmt_unfound;
        console_output << endl;
        ...
        if (save && (parentID_particletype+pmt_unfound) < 50) {
            ...
            console_output << "Number of PMT's not found/Incorrect particle/parent ID's greater than 50. Don't save";
            console_output << endl;
        }
