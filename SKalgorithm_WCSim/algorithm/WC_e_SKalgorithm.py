...
for opt, arg in opts:
    if opt == '-h':
        ...
        #object.
        try:
            FLAGS.set_parameters(WC_e_SKinput.ParameterReadWrite(arg).read())
        except:
...
def train(data_set):
    ...
    with tf.Graph().as_default():
        ...
        images, labels = WC_e_SKinput.input(data_set)
...
def test():
    ...
    with tf.Graph().as_default():
        ...
        images, sets, labels = WC_e_SKinput.input(0, shuffle=False)
        ...
        summary_images = WC_e_SKinput.get_summary_filter(sets, labels, correct)
        real_images = None
        ...
        info_paths = WC_e_SKinput.get_files(0, "wc_e_info")
        for info_path in info_paths:
            ...
            #Tensorboard output procedure
                ...
                if size >= FLAGS.num_iterations or info_paths.index(info_path) == len(info_paths) - 1:
                    ...
                    image_summary = WC_e_SKinput.get_summary(real_images)
                    WC_e_SKinput.write(sess, image_summary)
                    break
        ...
        if FLAGS.continue_session and not FLAGS.print_tensorboard:
            WC_e_SKinput.combine_info_files(complete_info_file, temp_info_file)
